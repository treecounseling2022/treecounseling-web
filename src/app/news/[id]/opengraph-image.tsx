...
export default async function Image({ params }: { params: Promise<{ id: string }> }) {
  ...
  const chars = `樹心理工作室treecounseling.com${title}${truncatedExcerpt}${category}`;
  ...
  return new ImageResponse(
    (
      ...
      >
        <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 8, background: "#3a4a3a", display: "flex" }} />
        {category ? (
          <div style={{ display: "flex", alignSelf: "flex-start", background: "#3a4a3a", color: "#f5f1ea", padding: "6px 20px", fontSize: 16, fontFamily: "Noto", letterSpacing: "0.05em" }}>
            {category}
          </div>
        ) : (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          <div style={{ fontSize: titleFontSize, color: "#1f2a24", fontFamily: "Noto", lineHeight: 1.3 }}>{title}</div>
          {truncatedExcerpt && (
            <div style={{ fontSize: 20, color: "#6b7c6b", fontFamily: "Noto", lineHeight: 1.6 }}>{truncatedExcerpt}</div>
          )}
        </div>
