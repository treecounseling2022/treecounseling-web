...
export async function GET(req: NextRequest) {
  ...
  for (const appt of appointments) {
    ...
    const sendReminderEmail = (recipientName: string, recipientEmail: string) =>
      resend.emails.send({
        from: FROM,
        to: recipientEmail,
    ...
    await sendReminderEmail(client.full_name, client.email);
    sent++;
    ...
    if (couplePartner.couple_session_type === "joint" && couplePartner.couple_partner?.email && couplePartner.couple_partner.email !== client.email) {
      await sendReminderEmail(couplePartner.couple_partner.full_name, couplePartner.couple_partner.email);
      sent++;
    }
  }
